...
function NavLi({ href, children }: { href: string; children: string }) {
  return (
    <li className={`group m-2 p-2 text-bold hover:text-purple-400`}>
      ...
      <Link
        href={href}
        className={`inline-flex text-nowrap duration-250 ease-in-out`}
      >
        {children}
      </Link>
    </li>
...
export default function SiteNav() {
  ...
  return (
    <nav
      className={
        "bg-neutral-950 py-2 px-4 flex items-center justify-between sticky top-0"
      }
    >
      <div className="flex items-center justify-between max-w-screen-lg w-full mx-auto">
        <Link href="/">
          <Image
            src="/images/sta-logo-full.svg"
            width={1000}
            height={100}
